...
>> Description
   Peacable Queens problem PDM: find the maximal A(n) such that A(n) white and A(n)
   black queens can be placed on a n x n chessboard without queens of opposite colors
   attacking each other. Solution score is the number of such attacks. A particular 
   problem instance is defined by Size and NPieces constants.
